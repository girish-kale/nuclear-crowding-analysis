...
// this only works because the data is already analyzed, and in the process the folder is already created
target=folder+"/nuclear domain size analysis/";
targets=getFileList(target);

file=0;
do{ // DO-WHILE loop through files 
	if (startsWith(files[file],"Dme_") && endsWith(files[file], ".tif")) { // IF statement to pick TIFF files
		open(folder+files[file]);		name=File.nameWithoutExtension;		//run("Maximize");

// A quick FOR loop to check if the file needs to be processed again. The ones that need to be processed are identified
// manually, and the 'stack' file is deleted. The missing 'stack' file then triggers the image analysis part of the code.
// Otherwise the image analysis part is skipped, and we go to the next timepoint and/or embryo
		nStacks=0;
		for (stacks=0; stacks<targets.length; stacks++){
			if (startsWith(targets[stacks], name) && endsWith(targets[stacks], "_stack.tif")){
				nStacks=nStacks+1;
			}
		}

		if (nStacks==0){ // IF statement to confirm that the file needs to be re-analyzed
...
// 2 z-slices. The minimun z-height would be 12um, which corresponds to height of early cellularization nuclei, while the maximum would be
...
// here we are testing different thresholds to see which gives best segmentation
			...
			stdevLevels=newArray("-1", "-0.75", "-0.5", "-0.25", "0", "0.25", "0.5");
			...
			do{ //loop to test different thresholds
				...
				default=4+stdev*4; // be sure to adjust this equation, as required, if the original stdevLevels array is altered,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				selectWindow(name);		run("Select None");		run("Duplicate...", "title=["+name+"_Maxima]");
				selectWindow(name+"_Maxima");		run("Multiply...", "value=0");
...
// then, cleaning the edges
				...
				run("Merge Channels...", "c1=["+name+"_Maxima] c4=["+name+"] create");
				...
				if (condition==0){
					run("Split Channels");
					selectWindow("C1-Composite");		rename(name+"_Maxima");		close(name+"_Maxima"); // unnecessarilty explicit, but anyway 
...
// Ideally, the added touch-ups should be of different size, as compared to automated ones generated above.
			...
			do{ // loop to manually correct the identified nuclei
				...
				selectWindow("C1-Composite");		rename(name+"_Maxima");
				selectWindow("C2-Composite");		rename(name);
...
// note that the criterion is objective
				...
				for (i=0; i<nResults; i++) { // FOR loop to filter out the outlier ROIs
					...
					if (getResult("Min", i)<5){ // any ROI that sticks out beyond the embryo is marked for deletion
				...
				run("Clear Results");
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				run("Merge Channels...", "c1=["+name+"_Maxima] c4=["+name+"] create");		roiManager("Show All without labels");
			...
			selectWindow(name+"_Maxima");		roiManager("deselect");		run("Remove Overlay");		setMinAndMax(0, 255);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		} // IF statement to confirm that the file needs to be re-analyzed
